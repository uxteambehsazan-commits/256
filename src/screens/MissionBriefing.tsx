...
export default function MissionBriefing({ state, dispatch }: Props) {
  ...
  const readyCount = activePlayers.filter(p => state.briefingReady[p.id]).length
      ...
      <div className="flex-1 overflow-y-auto px-5 flex flex-col gap-3 min-h-0">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {activePlayers.map(p => {
              const isReady = !!state.briefingReady[p.id]
              const color = PLAYER_COLORS[p.colorIndex % PLAYER_COLORS.length]
              return (
                <button
                  key={p.id}
                  onClick={() => !isReady && dispatch({ type: 'BRIEFING_READY', playerId: p.id })}
