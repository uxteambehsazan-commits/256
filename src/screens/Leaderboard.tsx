...
export default function Leaderboard({ state }: Props) {
  ...
  const isLast = state.currentMissionIndex >= MISSIONS.length - 1
    ...
    <div className="h-full flex flex-col items-center justify-center gap-4 p-4 relative overflow-hidden">
      ...
      <div className="animate-pop-in text-center">
        <div className="text-sm font-bold mb-1" style={{ color: '#CC2229' }}>
          {isLast ? '🏆 رتبه‌بندی نهایی' : `بعد از مأموریت ${state.currentMissionIndex + 1}`}
        </div>
        <h2 className="font-display text-3xl font-black text-white">جدول امتیازات</h2>
      </div>
      ...
      <div className="text-sm animate-pulse" style={{ color: '#CC2229' }}>
        {isLast ? '🏆 نمایش برنده...' : `مأموریت بعدی: ${MISSIONS[state.currentMissionIndex + 1]?.emoji} ${MISSIONS[state.currentMissionIndex + 1]?.name}`}
      </div>
    </div>
  )
}
