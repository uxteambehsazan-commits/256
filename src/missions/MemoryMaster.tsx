...
export default function MemoryMaster({ state, dispatch }: Props) {
  ...
  // Submit when timer hits 0 or board finished
  useEffect(() => {
    if (state.timeLeft === 0 && !done) {
      submitScore()
    }
  }, [state.timeLeft])
  ...
  const isRevealed = (idx: number) => board.matched.includes(idx) || tempFlipped.includes(idx)
